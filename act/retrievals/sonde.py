...
def calculate_pbl_bulk_richardson(
    ds,
    temperature='tdry',
    pressure='pres',
    rh='rh',
    ...
    base=5.0,
    thresholds=(0.25, 0.5),
    vapor_pressure_method='vap',
    max_pbl_height=4000.0,
):
    """
    Function for calculating the PBL height from a radiosonde profile using
    the bulk Richardson number technique.  The bulk Richardson number relates
    vertical stability to vertical shear, representing the ratio of thermally
    produced turbulence to that generated by vertical shear.  The method
    assumes there is no turbulence production at the top of the boundary
    layer, so the bulk Richardson number exceeds its critical value there.
    The PBL height is taken as the lowest height at which it reaches a
    critical threshold.

    The literature does not settle on a single critical threshold, so the ARM
    VAP reports heights for both 0.25 and 0.5.  Both are returned by default.
    ...
    temperature : str
        The name of the temperature field.
    pressure : str
        The name of the pressure field.  Expected to be in hPa.
    rh : str
        The name of the relative humidity field, in percent.
    u_wind : str
        The name of the eastward wind component field.
    v_wind : str
        The name of the northward wind component field.
    height : str
        The name of the height field
    smooth_height : int
        Number of points to do a moving average on sounding height data to reduce noise
    ...
    thresholds : tuple or list
        Critical bulk Richardson number thresholds to evaluate.  One PBL height
        variable is returned per threshold, named from the threshold value, so
        the default of (0.25, 0.5) produces 'pblht_bulk_richardson_pt25' and
        'pblht_bulk_richardson_pt5' to match the ARM VAP field names.
    vapor_pressure_method : str
        Method used to calculate saturation vapor pressure.  'vap' (default)
        reproduces Eq 12 of Sivaraman et al. (2013); 'metpy' uses
        metpy.calc.saturation_vapor_pressure.  See the note below.
    max_pbl_height : float
        Maximum PBL height AGL, in meters.  Following the VAP, heights above
        this are flagged as bad and set to -9999.

    Returns
    -------
    ds : xarray Dataset
        xarray dataset with results stored in pblht_bulk_richardson_* variables
    ...
    .. note::
        ...
        pressure assuming the latent heat of vaporization is constant.  That
        holds near the surface but degrades at the cold temperatures aloft,
        where it departs from the formulation used by MetPy by as much as
    ...
    if vapor_pressure_method not in ('vap', 'metpy'):
        raise ValueError("vapor_pressure_method must be either 'vap' or 'metpy'")
    ...
    # Get data.  Cast to float so the wind QC below can assign NaN
    pres = ds2[pressure].values.astype(float)
    temp = ds2[temperature].values.astype(float)
    relh = ds2[rh].values.astype(float)
    u = ds2[u_wind].values.astype(float)
    v = ds2[v_wind].values.astype(float)
    alt = ds2[height].values.astype(float)
    theta = ds2['potential_temperature'].values

    # Calculate AGL, using the lowest valid level as the surface reference
    if np.isnan(alt[0]):
        sfc_idx = np.where(~np.isnan(alt))[0][0]
    else:
        sfc_idx = 0
    agl = alt - alt[sfc_idx]

    # Windspeeds below 50m AGL that exceed 33.5 m/s are treated as missing
    fast_wind = np.where((agl < 50.0) & (np.sqrt(u**2.0 + v**2.0) > 33.5))[0]
    u[fast_wind] = np.nan
    v[fast_wind] = np.nan

    # Calculate saturation vapor pressure, in hPa
    temp_k = temp + 273.15
    if vapor_pressure_method == 'vap':
        lv_over_rv = (
            mpconsts.water_heat_vaporization.to('J/kg').magnitude
            / mpconsts.water_gas_constant.to('J/(kg K)').magnitude
        )
        es = 6.11 * np.exp(-lv_over_rv * (1.0 / temp_k - 1.0 / 273.15))
    else:
        es = mpcalc.saturation_vapor_pressure(temp * units.degC).to('hPa').magnitude

    # Partial pressure of water vapor and the resulting virtual temperature
    e = (relh / 100.0) * es
    temp_v = temp_k / (1.0 - (e / pres) * (1.0 - 0.622))

    # Use the same conversion as the dry potential temperature so that the two
    # profiles share an exponent
    theta_v = (
    ...
    # Calculate the bulk Richardson number.  At the surface the expression is
    # 0/0, so it is set explicitly
    gravity = mpconsts.earth_gravity.to('m/s^2').magnitude
    with np.errstate(divide='ignore', invalid='ignore'):
        rib = (gravity * agl / theta_v[sfc_idx]) * (
            (theta_v - theta_v[sfc_idx]) / (u**2.0 + v**2.0)
        )
    rib[sfc_idx] = 0.0

    # Find the lowest height reaching each threshold and add it to the dataset
    for threshold in thresholds:
        crossing = np.where(rib >= threshold)[0]
        crossing = crossing[crossing > sfc_idx]
        if len(crossing) == 0 or agl[crossing[0]] > max_pbl_height:
            pbl = -9999.0
        else:
            pbl = float(alt[crossing[0]])

        suffix = 'pt' + f'{float(threshold):g}'.lstrip('0').replace('.', '')
        atts = {
            'units': 'm',
            'long_name': (
                'Planetary Boundary Layer Height Bulk Richardson, '
                f'critical threshold {threshold}'
            ),
        }
        da = xr.DataArray(pbl, attrs=atts)
        ds[f'pblht_bulk_richardson_{suffix}'] = da
    ...
    atts = {'units': 'K', 'long_name': 'Gridded virtual potential temperature'}
    da = xr.DataArray(theta_v, coords={'atm_pres_ss': pres}, dims=['atm_pres_ss'], attrs=atts)
    ds['virtual_potential_temperature_ss'] = da

    atts = {'units': 'm', 'long_name': 'Gridded altitude'}
    da = xr.DataArray(alt, coords={'atm_pres_ss': pres}, dims=['atm_pres_ss'], attrs=atts)
    ds['alt_ss'] = da

    atts = {'units': '1', 'long_name': 'Bulk Richardson number'}
    da = xr.DataArray(rib, coords={'atm_pres_ss': pres}, dims=['atm_pres_ss'], attrs=atts)
    ds['bulk_richardson_number'] = da
