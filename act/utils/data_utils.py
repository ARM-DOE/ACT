...
def potential_temperature(obj, temperature, pressure):
    """
    Converts pressure for change in height
    ...
    ----------
    obj : xarray DataSet
        xarray dataset to get data from
    temperature : str, list
    ...
    -------
    obj : xarray DataSet
        Object with potential_temperature variable added

    """

    # Get variables and set constants
    temp = obj[temperature]
    temp = convert_units(temp.values, temp.attrs['units'], 'K')
    pres = obj[pressure]
    pres = convert_units(pres.values, pres.attrs['units'], 'hPa')

    p_0 = 1000.
    k = 0.286 #R/cpd

    # Calcluate potential temperature
    theta = temp * (p_0 / pres) ** k

    atts = {'units': 'K', 'long_name': 'Potential temperature'}
    da = xr.DataArray(theta, coords=obj[temperature].coords, dims=obj[temperature].dims, attrs=atts)
